...
class AudioSteam(object):
    def __init__(self):
        ...
        self.x_wave = np.arange(0, 2 * self.CHUNK, 2)
    ...
    def update(self):
        ...
        self.set_plotdata(name='waveform', data_x = self.x_wave, data_y=wf_data,)

        sp_data = fft(np.array(wf_data, dtype='int8') - 128)
        sp_data = np.abs(sp_data[0:int(self.CHUNK / 2)]) * 2 / (128 * self.CHUNK)
        self.set_plotdata(name='spectrum', data_x=self.x_spectrum, data_y=sp_data)
